...
export default function SettingsPage({ workspace, existingWorkspaceNames = [], storageUsedMb, onOpenApiKeys, onWorkspaceNameChange, canDeleteWorkspace, onDeleteWorkspace, onDirtyChange, onSaveSuccess, leaveRequest, onLeaveResolved }: SettingsPageProps) {
  ...
  const [activeSection, setActiveSection] = useState(() => { const requested = new URLSearchParams(window.location.search).get('section'); return visibleSections.some(([id]) => id === requested) ? requested! : visibleSections[0][0] })
  const contentRef = useRef<HTMLDivElement>(null)
  const [isDeleteOpen, setIsDeleteOpen] = useState(false)
  const [utilityDialog, setUtilityDialog] = useState<'sessions' | null>(null)
  const [isReviewOpen, setIsReviewOpen] = useState(false)
  const [reviewMode, setReviewMode] = useState<'manual' | 'leave'>('manual')
  ...
  const settingChanges = (Object.keys(settingLabels) as Array<keyof SavedSettings>).filter((key) => key === 'workspaceName' ? String(currentSettings[key]).trim() !== String(savedSettings[key]).trim() : currentSettings[key] !== savedSettings[key]).map((key) => ({ key: String(key), label: settingLabels[key], before: displaySettingValue(savedSettings[key]), after: displaySettingValue(currentSettings[key]) }))
  const memberChanges = [
  ...
  useEffect(() => { if (leaveRequest > 0 && isDirty) { setReviewMode('leave'); setIsReviewOpen(true) } }, [leaveRequest, isDirty])
  ...
  const saveAllChanges = () => {
    try {
      ...
      setSaveError(''); setIsReviewOpen(false); if (reviewMode === 'leave') onLeaveResolved(true)
  ...
  const cancelReview = () => {
    const shouldRestoreFocus = reviewMode === 'leave'
    setSaveError(''); setIsReviewOpen(false)
    if (reviewMode === 'leave') onLeaveResolved(false)
    if (shouldRestoreFocus) window.requestAnimationFrame(() => lastSettingsFocusRef.current?.focus())
  }
  ...
  return (
    <main className="settingsPage" aria-labelledby="settings-title" data-page="settings" onFocusCapture={(event) => { if (!isReviewOpen && event.target instanceof HTMLElement) lastSettingsFocusRef.current = event.target }}>
      ...
      <div className="settingsLayout">
        <nav className="settingsNav" aria-label="Settings sections">{visibleSections.map(([id, label]) => <button className={activeSection === id ? 'active' : ''} aria-current={activeSection === id ? 'page' : undefined} type="button" key={id} onClick={() => openSection(id)}>{label}</button>)}</nav>
        ...
        <div ref={contentRef} className="settingsContent">
          ...
          <section className="settingsGroup workspaceMembersGroup" id="members-access"><header><div><h2>Members and access · {members.length} {members.length === 1 ? 'member' : 'members'}</h2></div>{canManage && <button className="inviteMembersButton" type="button" aria-expanded={isInviteOpen} onClick={() => { setIsInviteOpen((open) => !open); setMemberError('') }}><img src="/assets/members-invite.svg" alt="" aria-hidden="true" />Invite members</button>}</header>
      ...
      {isDirty && <div className="settingsUnsavedBar" role="status"><span>{changes.length} unsaved {changes.length === 1 ? 'change' : 'changes'}</span><button type="button" onClick={() => { setReviewMode('manual'); setIsReviewOpen(true) }}><Icon src={icons.review} /><span>Review changes</span></button></div>}

      {isReviewOpen && <div className="newFolderBackdrop settingsReviewBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) cancelReview() }}><section className="settingsReviewModal" role="dialog" aria-modal="true" aria-labelledby="settings-review-title"><header><div><h2 id="settings-review-title">{reviewMode === 'leave' ? 'Unsaved changes' : 'Review changes'}</h2><p>{reviewMode === 'leave' ? 'Save your changes before leaving this page?' : 'Review what changed before saving this workspace.'}</p></div><button type="button" aria-label={reviewMode === 'leave' ? 'Stay on Settings' : 'Close review changes'} onClick={cancelReview}><ThemeIcon light={icons.reviewClose} dark="/assets/settings-review-close-dark.svg" /></button></header><ul>{changes.map((change) => <li key={change.key}><span>{change.label}</span><div><del>{change.before}</del><ThemeIcon light={icons.reviewArrow} dark="/assets/settings-review-value-dark.svg" /><strong>{change.after}</strong></div></li>)}</ul>{saveError && <p className="settingsReviewError" role="alert">{saveError}</p>}<footer>{reviewMode === 'leave' && <button type="button" onClick={cancelReview}>Stay</button>}<button type="button" onClick={undoAllChanges}><ThemeIcon light={icons.reviewUndo} dark="/assets/settings-review-undo-dark.svg" />{reviewMode === 'leave' ? 'Discard & continue' : 'Undo changes'}</button><button className="settingsReviewSave" type="button" onClick={saveAllChanges}><ThemeIcon light={icons.reviewSave} dark="/assets/settings-review-save-dark.svg" />{reviewMode === 'leave' ? 'Save & continue' : 'Save changes'}</button></footer></section></div>}
      ...
      {utilityDialog && <div className="newFolderBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setUtilityDialog(null) }}><section className="newFolderModal settingsUtilityModal" role="dialog" aria-modal="true" aria-labelledby="settings-utility-title"><header><div><Icon src={icons.monitor} /><h2 id="settings-utility-title">Active sessions</h2></div><button type="button" aria-label="Close active sessions dialog" onClick={() => setUtilityDialog(null)}><Icon src={icons.reviewClose} /></button></header><div className="settingsUtilityContent"><div><span>Current session</span></div><p>Other-device session management will be available after account authentication is connected.</p><footer><button type="button" onClick={() => setUtilityDialog(null)}>Okay</button></footer></div></section></div>}
