...
export default function BillingUsagePage({ workspaceId, workspaceName, storageUsedMb }: { workspaceId: string; workspaceName: string; storageUsedMb: number }) {
  ...
  const [billingCycle] = useState<BillingCycle>(initialBilling.billingCycle)
  ...
  useEffect(() => { if (drawerOpen) window.requestAnimationFrame(() => document.querySelectorAll('.drawerPlanCard button.planActionInactive').forEach((button) => button.removeAttribute('aria-disabled'))) }, [drawerOpen, billingCycle])
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') { if (cardDialogOpen) setCardDialogOpen(false); else if (cancelDialogOpen) setCancelDialogOpen(false); else if (drawerOpen) closeDrawer() } }
  ...
  return <main className="billingPage" aria-labelledby="billing-title">
    ...
    <div className="accountSettingsLayout">
      ...
      <div ref={contentRef} className="accountPageContent billingContent">
        ...
        <section className="profileGroup figmaReceipts" id="billing-receipts" aria-labelledby="billing-receipts-title"><header><h2 id="billing-receipts-title">Email receipts</h2></header>
          <div className="receiptPrimary"><div><span>Primary billing email</span><div className="receiptPrimaryRow"><div className="receiptValue"><div className="receiptValueMain"><span>{primaryReceiptEmail}</span><button type="button" disabled aria-label="Edit billing email unavailable" title="Billing email editing is coming soon"><img src="/assets/billing-pencil.svg" alt="" /></button></div></div><button className="receiptSendTest" type="button" onClick={() => notify(`Test receipt sent to ${primaryReceiptEmail}.`)}><img src="/assets/billing-send.svg" alt="" />Send test</button></div></div></div>
    ...
    {drawerOpen && <div className="subscriptionDrawerBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeDrawer() }}><aside className={`subscriptionDrawer${drawerStep === 'confirm' ? ' confirmStep' : ''}`} role="dialog" aria-modal="true" aria-labelledby="subscription-drawer-title" onClick={(event) => { if (drawerStep !== 'plans') return; const button = (event.target as HTMLElement).closest('button'); const plan = plans.find((candidate) => button?.textContent?.includes(candidate.name)); if (button && plan && plan.id !== currentPlanId) choosePlan(plan) }}><header><div>{drawerStep === 'confirm' && <button className="drawerBack" type="button" onClick={() => setDrawerStep('plans')}>←</button>}<img className="planModalIcon" src="/assets/plan-modal-icon.svg" alt="" /><h2 id="subscription-drawer-title">{drawerStep === 'plans' ? 'Choose your plan' : `Switch to ${selectedPlan?.name}`}</h2></div><button className="drawerClose" type="button" aria-label="Close subscription plans" onClick={closeDrawer}><img src="/assets/plan-modal-close.svg" alt="" /></button></header>
      {drawerStep === 'plans' ? <div className="drawerBody"><div className="drawerPlanGrid">{plans.map((plan, index) => { const isCurrent = plan.id === currentPlanId; return <article className={`drawerPlanCard${plan.id === 'pro' ? ' recommended' : ''}`} key={plan.id}><div><h3>{plan.name}</h3>{!isCurrent && plan.id === 'pro' ? <span className="recommendedBadge">Recommended</span> : null}</div><p>{plan.description}</p><div className="drawerPlanPrice"><strong>${priceFor(plan, billingCycle)}</strong><span>/ month</span></div><button className={!isCurrent ? 'planActionInactive' : undefined} type="button" disabled={isCurrent} aria-disabled={!isCurrent || undefined}>{isCurrent ? 'Current plan' : index < currentPlanIndex ? `Downgrade to ${plan.name}` : `Upgrade to ${plan.name}`}</button><ul><li>{plan.storage} storage</li><li>Files up to {plan.maxFile}</li><li>{plan.workspaces} workspaces</li><li>{plan.members} members</li><li>{plan.sharing}</li><li>{plan.history} history</li></ul></article>})}</div><section className="enterpriseBanner"><p>Need higher limits, advanced controls, or a plan built around your team’s needs?</p><button type="button" onClick={() => notify('Enterprise consultation requested.')}><img src="/assets/plan-contact.svg" alt="" />Talk to our team</button></section></div> : selectedPlan && <div className="drawerConfirm"><div className="drawerChangeSummary"><div><span>{currentPlan.name}</span><small>Current plan</small></div><span>→</span><div><span>{selectedPlan.name}</span><small>{selectedPlan.id === 'free' ? 'Starts September 19' : `${billingCycle === 'annual' ? 'Annual' : 'Monthly'} billing`}</small></div></div>{isDowngrade ? <div className="billingDowngradeNotice"><strong>No charge today</strong><p>Your current plan remains active until September 19. Existing files stay available after the change.</p></div> : <><div className="prorationIntro"><span>Prorated upgrade</span><p>You only pay for the 15 days remaining in this billing period.</p></div><dl className="prorationBreakdown"><div><dt>{selectedPlan.name} for remaining 15 days</dt><dd>{money(remainingPlanCharge)}</dd></div><div><dt>Unused time on {currentPlan.name}</dt><dd>−{money(unusedCredit)}</dd></div><div><dt>Tax</dt><dd>Calculated at checkout</dd></div><div className="prorationTotal"><dt>Due today</dt><dd>{money(dueToday)}</dd></div></dl></>}<div className="drawerRenewal"><span>Next renewal</span><strong>September 19, 2026 · {money(billingCycle === 'annual' ? priceFor(selectedPlan, billingCycle) * 12 : priceFor(selectedPlan, billingCycle))}</strong></div><div className="drawerPayment"><span>Payment method</span><strong>{card ? `${card.brand} ending in ${card.lastFour}` : 'No payment method'}</strong></div><p className="pricingDisclaimer">Final tax and proration are calculated securely by the billing provider before any charge.</p><footer><button type="button" onClick={() => setDrawerStep('plans')}>Back</button><button className="confirm" type="button" onClick={confirmPlanChange}>{isDowngrade ? 'Schedule downgrade' : card ? `Pay ${money(dueToday)} & upgrade` : 'Add card & continue'}</button></footer></div>}
    </aside></div>}
