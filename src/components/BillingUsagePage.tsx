...
export default function BillingUsagePage() {
  ...
  const [card, setCard] = useState<Card | null>({ brand: 'Visa', lastFour: '4242', expiry: '08/29' })
  ...
  return <main className="billingPage" aria-labelledby="billing-title">
    ...
    <div className="accountSettingsLayout">
      ...
      <div className="accountPageContent billingContent">
        {toast && <div className="billingSuccess" role="status">{toast}</div>}
        ...
        <section className="profileGroup figmaPaymentMethods" id="billing-payment" aria-labelledby="billing-payment-title"><header><h2 id="billing-payment-title">Payment methods</h2><button type="button" onClick={() => setCardDialogOpen(true)}>＋ Add more</button></header>{card ? <div className="billingPaymentCard"><div className="paymentCardIdentity"><img src="/assets/billing-visa.svg" alt="Visa" /><div><div><strong>{card.brand} ending in {card.lastFour}</strong><span className="paymentDefaultBadge">Default</span></div><small>Expires {card.expiry} · Default payment method</small></div></div><div><button type="button" onClick={() => setCardDialogOpen(true)}>Edit</button></div></div> : <div className="billingPaymentEmpty"><div><span>No payment method</span><small>Add a card before upgrading to a paid plan.</small></div><button type="button" onClick={() => setCardDialogOpen(true)}>Add payment method</button></div>}</section>
        ...
        <section className="profileGroup figmaBillingDetails" id="billing-details" aria-labelledby="billing-details-title"><header><h2 id="billing-details-title">Billing address &amp; tax ID</h2>{!isEditingBillingDetails && <button type="button" onClick={() => { setBillingDetailsDraft(billingDetails); setIsEditingBillingDetails(true) }}><img src="/assets/billing-details-edit.svg" alt="" />Edit</button>}</header>{isEditingBillingDetails ? <form className="billingDetailsForm" onSubmit={(event) => { event.preventDefault(); setBillingDetails(billingDetailsDraft); setIsEditingBillingDetails(false); notify('Billing details saved for future invoices.') }}><label>Full or company name<input required value={billingDetailsDraft.name} onChange={(event) => setBillingDetailsDraft({ ...billingDetailsDraft, name: event.target.value })} /></label><label>Country or region<select value={billingDetailsDraft.country} onChange={(event) => setBillingDetailsDraft({ ...billingDetailsDraft, country: event.target.value })}><option>Indonesia</option><option>Singapore</option><option>United States</option></select></label><label className="wide">Address<input required value={billingDetailsDraft.address} onChange={(event) => setBillingDetailsDraft({ ...billingDetailsDraft, address: event.target.value })} /></label><label>City<input required value={billingDetailsDraft.city} onChange={(event) => setBillingDetailsDraft({ ...billingDetailsDraft, city: event.target.value })} /></label><label>Postal code<input required value={billingDetailsDraft.postalCode} onChange={(event) => setBillingDetailsDraft({ ...billingDetailsDraft, postalCode: event.target.value })} /></label><label className="wide">Tax ID <span>Optional</span><input value={billingDetailsDraft.taxId} onChange={(event) => setBillingDetailsDraft({ ...billingDetailsDraft, taxId: event.target.value })} /></label><div className="wide billingFormActions"><button className="secondary" type="button" onClick={() => { setBillingDetailsDraft(billingDetails); setIsEditingBillingDetails(false) }}>Cancel</button><button type="submit">Save</button></div></form> : <div className="billingDetailsSummary"><div><span>Billing name</span><strong>{billingDetails.name}</strong></div><div><span>Address</span><strong>{billingDetails.address}<br />{billingDetails.city}, {billingDetails.postalCode}<br />{billingDetails.country}</strong></div><div><span>Tax ID</span><strong>{billingDetails.taxId || 'Not provided'}</strong></div></div>}</section>
...
function AddCardDialog({ onClose, onSave }: { onClose: () => void; onSave: (card: Card) => void }) {
  const [cardNumber, setCardNumber] = useState('')
  const [expiry, setExpiry] = useState('')
  const [cvc, setCvc] = useState('')
  const submit = (event: FormEvent) => { event.preventDefault(); const digits = cardNumber.replace(/\D/g, ''); if (digits.length < 4) return; onSave({ brand: digits.startsWith('4') ? 'Visa' : 'Mastercard', lastFour: digits.slice(-4), expiry: expiry || '08/29' }) }
  return <section className="billingConfirmModal addCardDialog" role="dialog" aria-modal="true" aria-labelledby="add-card-title"><header><div><span>Payment method</span><h2 id="add-card-title">Add a new card</h2></div><button type="button" aria-label="Close" onClick={onClose}>×</button></header><form onSubmit={submit}><label>Card number<input autoFocus required inputMode="numeric" autoComplete="cc-number" placeholder="4242 4242 4242 4242" value={cardNumber} onChange={(event) => setCardNumber(event.target.value)} /></label><div><label>Expiry<input required autoComplete="cc-exp" placeholder="MM/YY" value={expiry} onChange={(event) => setExpiry(event.target.value)} /></label><label>Security code<input required inputMode="numeric" autoComplete="cc-csc" placeholder="CVC" value={cvc} onChange={(event) => setCvc(event.target.value)} /></label></div><p>Prototype only. Production card collection will use a hosted payment provider.</p><footer><button type="button" onClick={onClose}>Cancel</button><button className="confirm" type="submit">Save payment method</button></footer></form></section>
}
